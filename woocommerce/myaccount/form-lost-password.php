...
if ( ! $is_send_password ) {
    ...
    <form method="post" action=""
          class="px-2 woocommerce-ResetPassword lost_reset_password container md-border md-border-border rounded-xs max-w-45 mx-auto pt-3/7 pb-6 lg-pb-3 md-mt-3/3 md-mb-9 flex flex-col">
        ...
        <p class="woocommerce-form-row woocommerce-form-row--first form-row form-row-first mb-3/5 flex flex-col">
            ...
            <input class="woocommerce-Input woocommerce-Input--text input-text w-405 text-base leading-2/2 pb-0/9 pt-1 border-2 border-border rounded-xs px-1/5"
                   type="number" name="user_login" id="user_login" autocomplete="username"/>
        </p>
        <p class="text-base leading-2/2">رمز عبور جدید به شماره شما ارسال خواهد شد.</p>
        <p class="woocommerce-form-row form-row mt-3/7">
            <input type="hidden" name="lost-password-nonce"
                   value="<?php echo esc_attr( wp_create_nonce( 'lost_password' ) ); ?>">
            <input type="submit" class="woocommerce-Button button cursor-pointer" name="send-password"
