		...
		<div class="relative w-full h-full pt-2/5 px-1/5 pb-2 flex flex-col justify-center items-center">
			...
			<div class="flex flex-col flex-1 relative to-single w-full">
				<div class="md-px-1">
					...
					?>
					<a href="<?php echo esc_attr( $product->get_permalink() ); ?>" class="block text-base leading-2/2 text-gray-dark text-center"><?php echo $product->get_title(); ?></a>
				</div>
				<div class="flex flex-col items-center flex-1 justify-end">
					...
					if ( $prices[ 'price' ] )
					{
						if ( $in_stock )
						{
							...
							<div class="text-base leading-2/8 text-gray-main text-center font-bold">
								<span class="text-lg text-gray-dark ml-0/3"><?php echo wc_price( $prices[ 'price' ] ); ?></span> تومان
							</div>
							<?php
						}
						else
					...
					else
					{
						...
						if ( ! $in_stock )
							$status = 'ناموجود';
						?>
						<div class="text-medium leading-2/8 text-gray-600 text-center font-bold"><?php echo $status; ?></div>
						<?php
					}
					?>
				</div>
				<div class="absolute top-16 left-half w-17 lg-flex items-center justify-center cubic-transition transform group-hover--translate-y-8 mt-0/3 -translate-x-1/2 custom-hidden-m">
					<a href="<?php echo esc_attr( $product->get_permalink() ); ?>" class="bg-primary-main text-white px-3/2 pt-0/8 pb-0/6 leading-2/2 text-base font-bold rounded-xs">مشاهده محصول</a>
				</div>
			</div>
			<div class="absolute top-1 lg-top-0 right-2 w-4 h-2 mr-0/4 text-primary-main">
				...
				if ( $prices[ 'discount' ] && $in_stock )
				{
					...
					<div class="relative icon-product-off leading-1/9 text-xl">
						<div class="text-center text-white leading-1/9 text-sm absolute top-0 right-0 z-10 w-full font-black" dir="ltr">-٪<?php echo $prices[ 'discount' ]; ?></div>
					</div>
					<?php
				}
				?>
