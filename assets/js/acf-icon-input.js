( function ( $ )
{
	...
	jQuery( document ).on( 'click', 'li[data-svg]', function ()
	{
		...
		active_item.parents( '.acf-icon-picker' ).find( '.acf-icon-picker__remove' ).addClass( 'acf-icon-picker__remove--active' );
	} );
	...
	function initialize_field ( $el )
	{
        ...
        <a class="acf-icon-picker__popup__close" href="javascript:">X</a>
        <h4 class="acf-icon-picker__popup__title">لطفا یک آیکون برگزینید</h4>
        <input class="acf-icon-picker__filter" type="text" id="filterIcons" placeholder="برای یافتن آیکون ها نام آنها را تایپ نمایید!" />
          ${list}
        </div>
      </div>`
			);
		...
		{
			$el.find( '.acf-icon-picker__remove' ).addClass( 'acf-icon-picker__remove--active' );
		}
		...
		$el.find( '.acf-icon-picker__remove' ).on( 'click', function ( e )
		{
			...
			parent.find( '.acf-icon-picker__svg' ).html( '<span class="acf-icon-picker__svg--span">+</span>' );

			jQuery( '.acf-icon-picker__img input' ).trigger( 'change' );

			parent.find( '.acf-icon-picker__remove' ).removeClass( 'acf-icon-picker__remove--active' );
		} );
	}
