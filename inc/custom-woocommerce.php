...
function fs_save_user_mobile_meta ( $customer_id )
{
	if ( isset( $_POST[ 'username' ] ) && ! empty( trim( $_POST[ 'username' ] ) ) )
	{
		...
		do_action( 'fs_welcome_sms', $customer_id );
	}
}
			...
			if ( $discount )
			{
				...
				<div class="text-primary-main absolute bottom-4 left-0 lg-static">
					<div class="relative icon-product-off leading-2/7 text-2/7">
						<div class="text-center text-white leading-2/7 text-xl absolute top-0 right-0 z-10 w-full font-black discount-percent" dir="ltr"><?php echo sprintf( '-٪%d', $discount ); ?></div>
					</div>
				</div>
				<?php
			}
