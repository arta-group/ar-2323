				...
				while ( have_rows( 'slider-categories', 'option' ) ) {
                    ...
                    <a href="<?php echo esc_url( get_category_link( $category->term_id ) ); ?>" class="w-12 mx-1/7">
                        <div class="w-full h-full flex flex-col items-center justify-center">
                            <img class="object-fit object-center w-8 h-8" src="<?php echo esc_url( $image ); ?>"
                                 alt="<?php echo ( isset($category->name) ) ? $category->name : ''; ?>"/>
                            <div class="text-base lg-text-medium font-bold mt-2/5 leading-2/5 text-gray-700 text-center"><?php echo ( isset($category->name) ) ? $category->name : ''; ?></div>
                        </div>
                    </a>
				<?php }
				?>
